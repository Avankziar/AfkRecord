package main.java.me.avankziar.afkr.velocity.ifh;
...
public class PlayerTimesProvider implements PlayerTimes
{
	...
	public PlayerTimesProvider(AfkR plugin)
	{
		this.plugin = plugin;
	}	
	...
	{
		return ((PluginUser) plugin.getMysqlHandler().getData(MysqlType.PLUGINUSER, 
				"`player_uuid` = ?", uuid.toString())).getPlayerName();
	}
	...
	public boolean addActiveTime(UUID uuid, long... time)
	{
		...
		TimeRecord tr = (TimeRecord) plugin.getMysqlHandler().getData(MysqlType.TIMERECORD, 
