...
public class AfkRCommandExecutor implements CommandExecutor
{
	...
	public AfkRCommandExecutor(AfkR plugin, CommandConstructor cc)
	{
		this.plugin = plugin;
		AfkRCommandExecutor.cc = cc;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String lable, String[] args) 
	{
		...
		{
			AfkR.logger.info("/%cmd% is only for Player!".replace("%cmd%", cc.getName()));
			return false;
		}
				...
				if(args[i].equalsIgnoreCase(ac.getName()))
				{
					if(length >= ac.minArgsConstructor && length <= ac.maxArgsConstructor)
					{
						if(player.hasPermission(ac.getPermission()))
						{
							...
							{
								AfkR.logger.info("ArgumentModule from ArgumentConstructor %ac% not found! ERROR!"
		...
		player.spigot().sendMessage(ChatApi.clickEvent(plugin.getYamlHandler().getLang().getString("InputIsWrong"),
				ClickEvent.Action.RUN_COMMAND, AfkR.infoCommand));
		return false;
	}
	...
	public void baseCommands(final Player player, int page)
	{
		...
		player.sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString(
				AfkR.infoCommandPath+".BaseInfo.Headline")));
		...
		pastNextPage(player, AfkR.infoCommandPath, page, lastpage, AfkR.infoCommand);
	}
