	...
	public void run(CommandSender sender, String[] args) throws IOException
	{
		...
		if(day <= 0)
		{
			new BukkitRunnable()
			{
				...
				public void run()
				{
					plugin.getCommandHelper().top(player, "alltime", pages, "CmdAfkRecord.Top.HeadlineAlT",
		...
		} else
		{
			new BukkitRunnable()
			{
				...
				public void run()
				{
					plugin.getCommandHelper().topLastXDays(player, "alltime", pages, "CmdAfkRecord.Top.HeadlineAlTLastDays",
							PluginSettings.settings.getCommands(KeyHandler.TOP_ALLTIME), days);
				}
			}.runTaskAsynchronously(plugin);
		}
	}
