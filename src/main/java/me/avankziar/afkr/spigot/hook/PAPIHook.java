package main.java.me.avankziar.afkr.spigot.hook;
...
public class PAPIHook extends PlaceholderExpansion
{
	...
	public String onPlaceholderRequest(Player player, String identifier)
	{
		...
		PluginUser user = (PluginUser) plugin.getMysqlHandler().getData(MysqlType.PLUGINUSER, "`player_uuid` = ?", player.getUniqueId().toString());
		if(user == null)
		{
			return "";
		}
		...
		case "user_30days_activitytime":
			...
			long sum = (long) plugin.getMysqlHandler().getSumII("player_uuid", "activitytime",
		...
		case "user_30days_afktime":
			...
			sum = (long) plugin.getMysqlHandler().getSumII("player_uuid", "afktime",
					"`player_uuid` = ? AND `timestamp_unix` >= ?", player.getUniqueId().toString(), beforeDays);
			return TimeHandler.getRepeatingTime(sum, format);
		case "user_30days_alltime":
			beforeDays = TimeHandler.getDate(TimeHandler.getDate(System.currentTimeMillis())) - 1000L*60*60*24*29;
			sum = (long) plugin.getMysqlHandler().getSumII("player_uuid", "alltime",
					"`player_uuid` = ? AND `timestamp_unix` >= ?", player.getUniqueId().toString(), beforeDays);
			return TimeHandler.getRepeatingTime(sum, format);
		case "user_365days_activitytime":
			...
			sum = (long) plugin.getMysqlHandler().getSumII("player_uuid", "activitytime",
		...
		case "user_toplist_place_activitytime":
			return String.valueOf(plugin.getMysqlHandler().getTopListPlaceI(
		...
		case "user_toplist_place_activitytime_with_format":
			return plugin.getUtility().getPlaceColor(plugin.getMysqlHandler().getTopListPlaceI(
					"`activitytime`", "`activitytime` > ?", user.getActiveTime()));
