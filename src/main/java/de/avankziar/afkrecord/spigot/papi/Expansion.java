...
public class Expansion extends PlaceholderExpansion
{
	...
	public String onPlaceholderRequest(Player player, String identifier)
	{
		...
		case "user_30days_activitytime":
			...
			//ADDME zu TimeHandler.getDateTime(String) machen.
		...
		case "user_toplist_place_activitytime_with_colorcode":
			return plugin.getUtility().getPlaceColor(plugin.getMysqlHandler().getTopListPlaceI(plugin,
					"`activitytime`", "`activitytime` > ?", user.getActivityTime()));
		case "user_toplist_place_afktime_with_colorcode":
			return plugin.getUtility().getPlaceColor(plugin.getMysqlHandler().getTopListPlaceI(plugin,
					"`afktime`", "`afktime` > ?", user.getAfkTime()));
		case "user_toplist_place_alltime_with_colorcode":
			return plugin.getUtility().getPlaceColor(plugin.getMysqlHandler().getTopListPlaceI(plugin,
					"`alltime`", "`alltime` > ?", user.getAllTime()));
		default:
			break;
	...
	public static void main(String[] args)
	{
		...
		String s = TimeHandler.getDate(now);
		System.out.println(s);
		System.out.println(TimeHandler.getDate(s));
		long n = TimeHandler.getDate(s) - 1000L*60*60*24*29;
		String b = TimeHandler.getDate(n);
		System.out.println(n);
		System.out.println(b);
		/*
		 * SELECT alltime, SUM(alltime) as Gesamttraffic FROM `SecretCraftAfkDateList`
		WHERE timestamp_unix > '1614808800000' AND player_name = 'Avankziar' GROUP BY player_name
		 */
	}
